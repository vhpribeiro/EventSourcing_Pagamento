...
namespace EventSourcing_Pagamento.Aplicacao.Test.Pagamentos
{
    public class ProcessamentoDePagamentoTeste
    {
        ...
        [InlineData("6771896725800366")]
        [InlineData("347143835400458")]
        public async Task Deve_aprovar_o_pagamento_de_um_cartao_valido_ao_criar_ao_reprocessar_um_pagamento(string numeroDeCartaoDeCreditoValido)
        {
            var id = _faker.Random.Int(0);
            var nomeDoCartao = _faker.Person.FullName;
            var produto = _faker.Random.String2(9);
        ...
        [InlineData("677186")]
        [InlineData("34754567")]
        public async Task Deve_negar_o_pagamento_de_um_cartao_valido_ao_criar_ao_reprocessar_um_pagamento(string numeroDeCartaoDeCreditoInvalido)
        {
            var id = _faker.Random.Int(0);
            var nomeDoCartao = _faker.Person.FullName;
            var produto = _faker.Random.String2(9);
